...
const moveCardTo = (dest, i, card) => {
  ...
  // console.log(state);
  console.log(`Moving card ${card} from ${location}[${pile}] to ${dest}[${i}]`);
};
...
function resetGame() {
  ...
  // Start at the winning condition

  //   for (let i = 0; i < 4; i++) {
  //     state.finish[i].cards = [];
  //     for (let j = 1; j <= 13; j++) {
  //       state.finish[i].cards.push(j + i * 13 - 1); // Adjust card numbers accordingly
  //       faceUp(j + i * 13 - 1); // Make sure cards are facing up
  //     }
  //   }
...
const gameFinish = () => {
  ...
  for (let i = 3; i >= 0; i--) {
    const l = state.finish[i].cards.length;
    console.log(`Pile ${i} has ${l} cards.`);
